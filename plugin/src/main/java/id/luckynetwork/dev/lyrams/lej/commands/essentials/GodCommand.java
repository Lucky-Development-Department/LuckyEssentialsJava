...
public class GodCommand extends CommandClass {
    ...
    public void godCommand(
        ...
        targets.forEach(target -> {
            ...
            target.sendMessage(Config.PREFIX + "§eGod mode: " + Utils.colorizeTrueFalse(godMode, TrueFalseType.ON_OFF) + "§e!");
        });

        if (others) {
            sender.sendMessage(Config.PREFIX + "§eToggled god for §d" + targets.size() + " §eplayers!");
        } else if (!(sender instanceof Player) || targets.doesNotContain((Player) sender)) {
            targets.stream().findFirst().ifPresent(target -> sender.sendMessage(Config.PREFIX + "§eGod mode for §d" + target.getName() + "§e: " + Utils.colorizeTrueFalse(target.hasMetadata("GOD"), TrueFalseType.ON_OFF) + "§e!"));
        }
    }
